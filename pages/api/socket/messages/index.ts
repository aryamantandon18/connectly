  ...
  try {
    ...
    const server = await db.server.findFirst({
      where: {
        ...
        members: {
          some: {
            profileId: profile.id,      // user sending this msg should be the part of server 
          },
        },
      },
      include: {
    ...
    res?.socket?.server?.io.emit(channelKey, message);  // emiting a socket.io event to all the active users
